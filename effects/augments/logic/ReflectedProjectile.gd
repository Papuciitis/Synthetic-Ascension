...
func _process(dt: float) -> void:
	...
	if (
		...
		EnemyCombat.apply_damage(hit_handle, damage, 1, source, BalanceAttribution.provenance("augment:reflect_shield", "augment:reflect_shield:reflected_projectile", "augment"))
		_despawn()
		return
	if world_hit_t >= 0.0:
		_despawn()
