...
# A bag copy carrying a different Manifestation still FEEDS the worn copy
# (rarity is never blocked); the worn rule wins and the dissolved rule is
# kept as an imprint for the Hub's imprinter.
func _test_equip_same_id_with_other_manifestation_feeds_and_keeps_imprint() -> void:
	_world_drops.clear()
	Global.attempt_imprints.clear()
	var inv := Inventory.new()
	var bag := BagInventory.new()
	var ring_rules: Array = ManifestationCatalog.pool_for_slot(ItemData.EquipSlot.RING)
	var rule_a: StringName = ring_rules[0].id
	var rule_b: StringName = ring_rules[1].id
	var worn := _make_item("ring_rule", ItemData.EquipSlot.RING)
	worn.manifestation_id = rule_a
	var copy := _make_item("ring_rule", ItemData.EquipSlot.RING)
	copy.manifestation_id = rule_b
	inv.set_item(ItemData.EquipSlot.RING, worn)
	bag.set_at(0, copy)
	var progress_before: int = worn.progress
	var ok := InvRouter.equip_from_bag(bag, 0, inv)
	_check(ok, "equipping a same-id copy with a different Manifestation succeeds")
	_check(inv.get_at(ItemData.EquipSlot.RING) == worn and worn.progress == progress_before + 1 and _total(inv, bag) == 1, "and FEEDS the worn copy: one instance, its mass grew")
	_check(worn.manifestation_id == rule_a and Global.attempt_imprints.has(rule_b), "the worn rule wins and the dissolved rule is held as an imprint")
	var inv2 := Inventory.new()
	var bag2 := BagInventory.new()
	var bare := _make_item("ring_rule", ItemData.EquipSlot.RING)
	var ruled := _make_item("ring_rule", ItemData.EquipSlot.RING)
	ruled.manifestation_id = rule_b
	inv2.set_item(ItemData.EquipSlot.RING, bare)
	bag2.set_at(3, ruled)
	var moved := InvRouter.move_between(bag2, 3, inv2, ItemData.EquipSlot.RING)
	_check(moved and inv2.get_at(ItemData.EquipSlot.RING) == bare and bare.manifestation_id == rule_b and _total(inv2, bag2) == 1, "a rule-less worn copy adopts the incoming rule when fed")
	Global.attempt_imprints.clear()
...
func _run() -> void:
	...
	_test_equip_same_id_with_other_manifestation_feeds_and_keeps_imprint()
