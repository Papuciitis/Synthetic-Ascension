...
const DATA: Dictionary = {
	&"enemy_containment_construct": {"name": "Lattice Construct", "quote": "The first stable Pattern is already trying to move.", "role": "Controlled synthesis", "behaviour": "Advances directly once released from its calibration field.", "expect": "A readable first combatant with no institutional reinforcements.", "counter": "Keep moving and test the attack method selected for this run."},
	&"enemy_opening_officer": {"name": "Containment Officer", "quote": "Drop the conduit. This does not need to become an execution.", "role": "Institutional security", "behaviour": "Remains non-hostile until the research is resisted, then closes distance.", "expect": "Your first lethal human confrontation.", "counter": "The decision to fire is yours; movement creates room once combat begins."},
	...
	&"enemy_splitter": {"name": "Splitter", "quote": "Every fragment still wants out.", "role": "Escalating swarm", "behaviour": "A large slow body divides into faster, smaller generations when destroyed.", "expect": "One neglected target can become a crowd of mobile fragments.", "counter": "Keep open space and clear each generation before engaging another pack."},
	...
	&"enemy_chanter": {"name": "Chanter", "quote": "Hold the line; the line will be mended.", "role": "Support caster", "behaviour": "Keeps its distance and chants: nearby forces are mended and warded every few seconds.", "expect": "Wounded enemies come back above your execute lines and shrug off a share of your hits.", "counter": "Silence the chant first; it has no attack of its own."},
	&"enemy_lurker": {"name": "Lurker", "quote": "It waits for the moment you stop.", "role": "Ambusher", "behaviour": "Hangs back at a distance while you move; the moment you stand still or channel, it charges.", "expect": "Every deliberate stop, every held cast, every Rite channel invites it.", "counter": "Keep moving, or kill it during its wind-up; it will not chase a moving target."},
	&"enemy_warden": {"name": "Warden", "quote": "Nothing passes the line.", "role": "Bulwark", "behaviour": "Advances slowly behind a shield sector that swallows bolts, impacts and fragments from the front; it turns slowly and drops the shield after eight hits or one from behind.", "expect": "Straight-line ranged and magic builds stop working on it.", "counter": "Get behind it, hit it in melee, or bait eight hits and strike in the gap."},
	&"enemy_siphon": {"name": "Siphon", "quote": "It drinks what you were saving.", "role": "Resource eater", "behaviour": "Clings like a Leech, but drains the discipline you are banking: Force, Momentum or Heat, eight a bite.", "expect": "A held meter empties while it is attached; a build with no meter loses Followers instead.", "counter": "Spend the meter before it can, or kill it on approach."},
