...
- `schema_version` 2 captures carry `metadata.recorder_revision` (the
  ...
  `balance_revision` (1 for the pre-curve item formula, 2 once the item scaling profiles are live), `tuning_stages` (`["items"]` when they are), `tuning_hash` (SHA-256 of `data/items/item_scaling_v2.json` when a valid profile exists, empty otherwise) and `metadata.features`: which measurements this recorder
