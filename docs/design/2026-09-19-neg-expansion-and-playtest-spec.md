...
| A3 Equilibrium Sigil, A5 Litany of Wounds, A6 Gravemarch polarity rule, A7 Gambler's Rite | **Built 2026-09-19** with the numbers of section 2.1: `Augment_EquilibriumSigil/LitanyOfWounds/GamblersRite.tres`, `BurdenResolver.equilibrium_bonus / litany_haste / gambler_follower_chance`, `LitanyOfWoundsEffect` through `AugmentRunner.get_haste_multiplier`, `GamblersRite` on the item operation events, `SetRunner._cursed_ballast` with `GravemarchCursedBallast`, the merge law's local deepen rule, the pickup gate and the Run Sheet lines. | `NegArchetypesTest` (60 checks) |
...
All four are built (see section 1). Sheet lines exist for each; the
observability fixes of section 2.3 still apply to the three older ones.
