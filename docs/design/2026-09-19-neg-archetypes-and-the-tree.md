...
| T3 | **Applied** (`LUCK_ROLL_SHIFT`, `AscensionSharedRulesTest`). `AscensionRunner.roll`: add `LuckResolver.effective(Global.run_luck) x 0.05` before the cap, so Luck moves named rolls by at most 5 points either way. | Finding 3. | `AscensionSharedRulesTest`: a -95% Luck wardrobe fails 5 points more often; Distortion pure under Jinxed Coin loses output. |
| T4 | **Applied** (`player.SHOT_HASTE_CAP` 2.5, `clamp_shot_haste`). A global cap on the shot haste multiplier in `_fire_weapon` (x2.5), applied after every runner multiplies in. | Fever Litany x Burst x Litany x Overclock has no ceiling. | Test: stacked multipliers clamp; Barrage pure output with all four unchanged below the cap. |
