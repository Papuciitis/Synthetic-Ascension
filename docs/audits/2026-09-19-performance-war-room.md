...
| M8 | **Per-hit overhead** (from section 2.5; first step built: `BalanceAttribution.from_tags` memoized, ledger 7.15 to 6.90 us per hit) | Cheapen the fixed cost every `apply_damage` pays: memoize `BalanceAttribution.from_tags` by tag signature (a hit's tags repeat thousands of times per run), coalesce BattleText to one number per handle per frame, batch knockback and stun into the world arrays instead of per-handle calls, and let the recorder's `enemy_damage` skip the per-enemy row work for handles it has already seen this window. | Build simulator, Lattice pinned, melee only, seed 77: strike step p95 from 6.35 ms to under 4 ms with identical kills and HP removed; `BalanceRecorderLoadTest` ledger cost from 7.2 us per hit toward 4; `BalanceAttributionTest`, `BalanceLedgerTest` green. | low-medium: attribution and ledger totals must not change (the tests compare them); BattleText coalescing changes what the player sees on multi-hit frames. |
