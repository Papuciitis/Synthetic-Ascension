...
> **Status 2026-08-29:** the HIGH below is fixed in `e549847` — but not with the
> one-liner suggested: `PhysicsServer2D.is_flushing_queries()` does not exist in
> Godot 4.7.1. `projectile.gd` now defers `_despawn` out of its own
> `area_entered` and carries a `_hit` guard (`PooledProjectileRecycleTest`).
> Also: the pooled `projectile.tscn` is obtained only by the dead `Weapon.gd`;
> MagicMissile pools `MagicMissileProjectile.tscn`, a Node2D that despawns from
> `_physics_process`. The bug was latent, not live.
