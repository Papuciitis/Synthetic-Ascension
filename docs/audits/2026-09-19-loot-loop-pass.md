...
| L6 | **Done** (the router swaps instead of feeding when the worn copy would dissolve the bag copy's Manifestation; merges stay a deliberate bag action). Equip-feed with a differing Manifestation asks, or routes to the bag like a pickup (`allow_rule_loss = false` in the router; the Hub and inventory screens show a confirm). | Silent destruction of a Manifestation (section 4). | `InventoryRouterTest`: same-id, different-Manifestation equip lands in the bag. |
